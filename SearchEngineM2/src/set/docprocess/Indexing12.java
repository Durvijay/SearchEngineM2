...
public class Indexing12 {
	...
	public String processWord(String next) {
		...
		return pStem.processToken(next.replaceAll("^[^\\p{L}\\p{Nd}]+|[^\\p{L}\\p{Nd}]+$", "").toLowerCase());
	}
	/**
	...
	public void addTermInvertedIndex(String term, int docID, int wordPosition) {
		...
		try {
			...
			Logger.getLogger(Indexing12.class.getName()).log(java.util.logging.Level.SEVERE, null,e);
